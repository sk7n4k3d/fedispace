...
class _SearchPageState extends State<SearchPage>
  ...
  Future<void> _loadDiscover() async {
    try {
      ...
      final results = await Future.wait([
        widget.apiService.getTrendingPosts(limit: 20).catchError((e) {
          appLogger.error('Error loading trending posts', e);
          return <Status>[];
        }),
        widget.apiService.discoverPosts(limit: 30).catchError((e) {
          appLogger.error('Error loading discover posts', e);
          return <Status>[];
        }),
        widget.apiService.discoverPopularAccounts(limit: 10).catchError((e) {
          appLogger.error('Error loading popular accounts', e);
          return <Account>[];
        }),
        widget.apiService.discoverTrendingHashtags(limit: 10).catchError((e) {
          appLogger.error('Error loading trending hashtags', e);
          return <Map<String, dynamic>>[];
        }),
        widget.apiService.getSuggestions(limit: 8).catchError((e) {
          appLogger.error('Error loading suggestions', e);
          return <Account>[];
        }),
        widget.apiService.getFollowedTags(limit: 20).catchError((e) {
          appLogger.error('Error loading followed tags', e);
          return <Map<String, dynamic>>[];
        }),
        widget.apiService.discoverNetworkTrending(limit: 20).catchError((e) {
          appLogger.error('Error loading network trending', e);
          return <Status>[];
        }),
      ]);

      final trending =
          results[0] is List<Status> ? results[0] as List<Status> : <Status>[];
      final discover =
          results[1] is List<Status> ? results[1] as List<Status> : <Status>[];
      final popularAccounts = results[2] is List<Account>
          ? results[2] as List<Account>
          : <Account>[];
      final hashtags = results[3] is List
          ? List<Map<String, dynamic>>.from(results[3] as List)
          : <Map<String, dynamic>>[];
      final suggestionAccounts = results[4] is List<Account>
          ? results[4] as List<Account>
          : <Account>[];
      final followedTags = results[5] is List
          ? List<Map<String, dynamic>>.from(results[5] as List)
          : <Map<String, dynamic>>[];
      final networkTrending =
          results[6] is List<Status> ? results[6] as List<Status> : <Status>[];

      final popular = popularAccounts
          .map((a) => AccountUsers(
                id: a.id ?? '',
                username: a.username ?? '',
      ...
      final suggestions = suggestionAccounts
          .map((a) => AccountUsers(
                id: a.id ?? '',
                username: a.username ?? '',
                displayName: a.display_name ?? '',
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: RadialGradient(colors: [
                      CyberpunkTheme.neonCyan
                          .withOpacity(0.06 + _pulseController.value * 0.03),
                      Colors.transparent,
                    ]),
                  ),
                );
              ...
              children: [
                  ...
                  child: Container(
                    ...
                    child: TextField(
                      ...
                      style: const TextStyle(
                          color: CyberpunkTheme.textWhite, fontSize: 16),
                      decoration: InputDecoration(
                        hintText: S.of(context).searchHint,
                        hintStyle: TextStyle(
                            color: CyberpunkTheme.textTertiary, fontSize: 16),
                        prefixIcon: Icon(
                ...
                Expanded(
                    child: _hasSearched
                        ? _buildSearchResults()
                        : (_isGridView
                            ? ExploreGrid(apiService: widget.apiService)
  ...
  Widget _buildSearchResults() {
    ...
    final hasResults = _accountResults.isNotEmpty ||
        _statusResults.isNotEmpty ||
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.search_off_rounded,
                  size: 32, color: CyberpunkTheme.textTertiary),
            ),
            ...
            Text(S.of(context).tryDifferentKeywords,
                style: const TextStyle(
              ...
              children: [
                Container(
                  ...
                  child: const Icon(Icons.tag_rounded,
                      size: 18, color: CyberpunkTheme.neonCyan),
                ),
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
            ...
            Wrap(
              ...
              children: _followedTags.take(12).map((t) {
                ...
                return GestureDetector(
                  onTap: () => Navigator.pushNamed(context, '/TagTimeline',
                      arguments: {'tag': name}),
                  child: Container(
                    padding:
          ...
          if (hasHashtags) ...[
            ...
            Wrap(
              ...
              children: _trendingHashtags.take(8).map((t) {
                ...
                final uses =
                    t['history'] is List && (t['history'] as List).isNotEmpty
                    ...
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
                        colors: [
                          ...
                          CyberpunkTheme.pinkMuted.withOpacity(0.1)
                        ],
                      ),
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(
          ...
          if (hasPopular) ...[
            _sectionHeader(
                S.of(context).explore, Icons.local_fire_department_rounded),
            SizedBox(
              height: 110,
              child: ListView.builder(
                ...
                itemBuilder: (ctx, i) =>
                    _buildPopularAccountCard(_popularAccounts[i]),
              ),
            ),
            const SizedBox(height: 20),
          ],
          ...
          if (hasSuggestions) ...[
            ...
            SizedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (ctx, i) =>
                    _buildPopularAccountCard(_suggestions[i]),
              ),
            ),
            const SizedBox(height: 20),
          ],
        ...
        child: Column(
          children: [
            ...
            Text(
              '@${account.username}',
              style: const TextStyle(
                  fontSize: 10, color: CyberpunkTheme.textTertiary),
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
              textAlign: TextAlign.center,
            ),
  ...
  Widget _buildPostGrid(List<Status> posts) {
    return GridView.builder(
      ...
      itemBuilder: (ctx, i) {
        final post = posts[i];
        final imageUrl =
            post.attach.isNotEmpty ? post.attach : _getFirstMediaUrl(post);
        if (imageUrl.isEmpty) {
          return Container(
            color: CyberpunkTheme.cardDark,
            child: const Center(
            ...
            child: CachedNetworkImage(
              ...
              errorWidget: (_, __, ___) => Container(
                ...
                child: const Icon(Icons.broken_image_rounded,
                    color: CyberpunkTheme.textTertiary, size: 20),
              ),
            ),
          ),
        );
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(S.of(context).findYourPeople,
              style: const TextStyle(
                  fontSize: 20,
                  fontWeight: FontWeight.w700,
                  color: CyberpunkTheme.textWhite)),
          const SizedBox(height: 8),
          Text(S.of(context).searchByUsername,
              style: const TextStyle(
