...
class ContentFiltersPage extends StatefulWidget {
  ...
  const ContentFiltersPage({Key? key, required this.apiService})
      : super(key: key);
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          title: Text(S.of(context).newFilter,
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    _filterChip('Warn', selectedAction == 'warn',
                        () => setDialogState(() => selectedAction = 'warn')),
                    const SizedBox(width: 8),
                    _filterChip('Hide', selectedAction == 'hide',
                        () => setDialogState(() => selectedAction = 'hide')),
                  ],
                ),
                const SizedBox(height: 12),
                Text(S.of(context).applyTo,
                    style: const TextStyle(
                        color: CyberpunkTheme.textSecondary, fontSize: 12)),
                const SizedBox(height: 4),
                Wrap(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: CyberpunkTheme.backgroundBlack,
        elevation: 0,
        title: Text(S.of(context).contentFilters,
            style: const TextStyle(
      ...
      body: _isLoading
          ...
          : _filters.isEmpty
              ? Center(
                  child: Text(S.of(context).noContentFilters,
                      style: const TextStyle(
              ...
              : RefreshIndicator(
                  ...
                  child: ListView.separated(
                    ...
                    separatorBuilder: (_, __) => const Divider(
                        color: CyberpunkTheme.borderDark, height: 1),
                    itemBuilder: (context, index) {
                      final filter = _filters[index];
                      final title = filter['title'] ?? 'Untitled';
                      final action = filter['filter_action'] ?? 'warn';
                      final contexts =
                          (filter['context'] as List?)?.join(', ') ?? '';
                      final keywords = (filter['keywords'] as List?)
                              ?.map((k) => k['keyword'] ?? '')
                              .join(', ') ??
                          '';
                      ...
                      return ListTile(
                        ...
                        trailing: IconButton(
                          icon: const Icon(Icons.delete_outline,
                              color: CyberpunkTheme.textTertiary, size: 18),
                          onPressed: () async {
