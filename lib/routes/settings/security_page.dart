...
class _SecurityPageState extends State<SecurityPage> {
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  _sectionHeader(
                      'Recent Login Activity', Icons.history_rounded),
                  _loginActivityList(),
                  const SizedBox(height: 24),
      ...
      child: Row(children: [
        ...
        ElevatedButton(
          onPressed: () => ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                  content: Text('2FA setup requires your instance web UI'))),
          style: ElevatedButton.styleFrom(
            backgroundColor:
                enabled ? CyberpunkTheme.cardDark : CyberpunkTheme.neonCyan,
  ...
  Widget _loginActivityList() {
    if (_loginActivity.isEmpty) {
      return _glassCard(
          child: const Center(
              child: Text('No login activity data available',
                  style: TextStyle(color: CyberpunkTheme.textSecondary))));
    }
    return _glassCard(
        child: Column(
      children: _loginActivity
          .take(10)
          .map((s) => Padding(
                padding: const EdgeInsets.symmetric(vertical: 8),
                child: Row(children: [
                  const Icon(Icons.devices_rounded,
                      color: CyberpunkTheme.textSecondary, size: 20),
                      ...
                      child: Column(
                        ...
                        Text(
                            s['device']?.toString() ??
                                ...
                                'Unknown device',
                            style: const TextStyle(
                                color: CyberpunkTheme.textWhite, fontSize: 13),
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis),
                        Text(
                            '${s['ip'] ?? 'Unknown IP'} - ${s['created_at'] ?? ''}',
                            style: const TextStyle(
  ...
  Widget _authorizedAppsList() {
    if (_authorizedApps.isEmpty) {
      return _glassCard(
          child: const Center(
              child: Text('No authorized apps found',
                  style: TextStyle(color: CyberpunkTheme.textSecondary))));
    }
    return _glassCard(
        child: Column(
      children: _authorizedApps
          .map((app) => Padding(
                padding: const EdgeInsets.symmetric(vertical: 8),
                child: Row(children: [
                  ...
                  const SizedBox(width: 12),
                  Expanded(
                      child: Text(app['name']?.toString() ?? 'Unknown app',
                          style: const TextStyle(
  ...
  Widget _changePasswordForm() {
    return _glassCard(
      ...
      TextField(
          ...
          obscureText: !_showPasswords,
          decoration: const InputDecoration(
              labelText: 'New password', prefixIcon: Icon(Icons.lock_rounded))),
      const SizedBox(height: 12),
      TextField(
      ...
      Row(children: [
        Switch(
            ...
            onChanged: (v) => setState(() => _showPasswords = v)),
